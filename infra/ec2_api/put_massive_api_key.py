"""Store MASSIVE_API_KEY from local .env into SSM as a SecureString.

Never prints the key. Run from the repo root:

    AWS_PROFILE=trade-recon-8948 uv run python infra/ec2_api/put_massive_api_key.py
"""
...
def _read_key(env_path: Path) -> str:
    if not env_path.is_file():
        raise FileNotFoundError(f"Missing {env_path}; copy .env.example and set MASSIVE_API_KEY")
    found: str | None = None
